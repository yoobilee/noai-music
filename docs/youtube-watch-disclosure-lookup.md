...
공식 disclosure가 카드 DOM에 직접 없을 때 카드당 작은 배지 하나의 text와 색만 갱신한다.

- `NoAI dev: checking disclosure`
- `NoAI dev: disclosure detected`
- `NoAI dev: no disclosure detected`
- `NoAI dev: disclosure status unknown`

기존 카드 자체에 공식 disclosure가 있으면 기존 직접 감지 배지를 우선하고 추가 확인하지 않는다. watch-page 본문 candidate도 다시 fetch하지 않는다. 어떤 상태도 아직 숨김·흐림·허용·차단 결정에 사용하지 않는다.
...
3. 홈 또는 검색에서 공개 disclosure 표본 카드가 보이면 `NoAI dev: checking disclosure` 뒤 detected로 바뀌는지 확인한다.
4. 일반 카드가 not detected로 바뀌고 콘텐츠가 숨겨지거나 흐려지지 않는지 확인한다.
...
- 개발 상태 배지는 제품 UI가 아니며 실제 필터, allow/block, 자동 skip을 구현하지 않는다.
